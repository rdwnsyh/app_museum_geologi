...
const Pinjam = ({ cartItems }) => {
    const { data, setData, post, errors, processing } = useForm({
        items: cartItems, // Menyertakan cartItems yang dipilih
        tanggal_pinjam: '',
        tanggal_jatuh_tempo: '',
    });
    ...
    return (
        <div className="flex flex-col min-h-screen">
            ...
            <div className="container mx-auto p-4 flex-grow">
                ...
                <form onSubmit={handleSubmit} className="bg-white shadow p-4 rounded-md">
                    <div className="grid gap-4">
                        <div>
                            <label className="block font-semibold">Tanggal Pinjam</label>
                            <input
                                type="date"
                                name="tanggal_pinjam"
                            ...
                            {errors.tanggal_pinjam && (
                                <div className="text-red-600">{errors.tanggal_pinjam}</div>
                            )}
                        </div>

                        <div>
                            <label className="block font-semibold">Tanggal Jatuh Tempo</label>
                            <input
                                type="date"
                            ...
                            {errors.tanggal_jatuh_tempo && (
                                <div className="text-red-600">{errors.tanggal_jatuh_tempo}</div>
                            )}
                        </div>

                        <div>
                            <h3 className="font-semibold">Items yang Dipilih</h3>
                            <ul>
                                {data.items.map((item) => (
