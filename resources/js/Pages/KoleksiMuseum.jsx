                ...
                <div className="space-y-4">
                    {filteredResults.length > 0 ? (
                        filteredResults.map(item => (
                            <div key={item.id} className="border p-4 rounded-lg shadow flex items-center">
                                <div className="flex-grow">
                                    <h2 className="text-xl font-semibold">
                                        <a
                                            href={`/detail/${item.id}/batuan`} // Update the link to point to the detailkoleksi page
                                            className="text-blue-500 hover:underline"
                                        >
                                            {item.nama_koleksi}
                                        </a>
                                    </h2>
                                    <p className="text-gray-500 mt-1">{item.deskripsi_koleksi || "Deskripsi tidak tersedia."}</p>
                                    <a
