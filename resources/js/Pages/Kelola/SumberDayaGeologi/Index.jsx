...
function Index() {
    const { koleksi_sdg } = usePage().props;

    const data = koleksi_sdg?.data || [];
    const links = koleksi_sdg?.meta?.links || [];

    const handleDelete = (id) => {
        if (window.confirm("Are you sure you want to delete this item?")) {
            router.delete(route("kelolakoleksisumberdayageologi.destroy", id), {
                onSuccess: () => {
            ...
            <div className="flex items-center justify-between mb-2">
                ...
                <div className="flex items-center justify-end mb-2">
                    <Link
                        ...
                        href={route("kelolakoleksisumberdayageologi.create")}
                    >
                    ...
                    { label: "Alamat Storage", name: "alamat" },
                    {
                        ...
                        renderCell: (row) => (
                            <div className="flex space-x-2">
                                <Link
                                    href={route(
                                        "kelolakoleksisumberdayageologi.edit",
                                        row.id
